...
Qiita / Qiita Team の記事を**ターミナルから読み書き**するための、依存パッケージゼロの小さな CLI です。
記事をローカルにダウンロード・同期せず、API を直接叩いて読み書きします。Node.js 標準モジュールと `fetch` のみで動きます。
...
- 一覧/閲覧: `GET /api/v2/authenticated_user/items`, `GET /api/v2/items/:id`
- 投稿/更新: `POST /api/v2/items`, `PATCH /api/v2/items/:id`
- タグ/グループ検索は全ページを取得してクライアント側で絞り込みます。
